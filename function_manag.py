...
def merge_two_funcs(dest: Function, source: Function):
    if len(source.labels) > 1:
        LOGGER.error("The targeted function has more than 1 lables")
    else:
        substitute(dest, source, 0, 0, 0)
...
def substitute(dest:Function, source: Function, lbl_i, op_i, counter_f):
    for l in range(lbl_i, len(dest.labels)):
        for op in range(op_i, len(dest.labels[l].operations)):
            if dest.labels[l].operations[op].name == source.name:
                ...
                put_func(dest, source, l, op, src, counter_f)
                return

def put_func(dest, source, lbl_i, op_i, src, counter_f):
    ...
    for op in src.labels[0].operations:
        ...
        op_i += 1
    substitute(dest, source, lbl_i, op_i, counter_f=counter_f+1)